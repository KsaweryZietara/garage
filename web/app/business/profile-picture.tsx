...
const ProfilePictureScreen = () => {
    ...
    const [profilePictureSubmitted, setProfilePictureSubmitted] = useState<boolean>(false);
    ...
    const handleSaveProfilePicture = async () => {
        setErrorMessage("")
        if (!image) {
            setErrorMessage("Wybierz zdjęcie profilowe.")
            return;
        }
        ...
        try {
            ...
            await axios.post("/api/employees/profile-picture",
                {profilePicture: base64Image},
                {headers: {"Authorization": `Bearer ${token}`}})

            setProfilePictureSubmitted(true);
        } catch (error) {
            setErrorMessage("Zapisanie zdjęcia profilowego nie powiodło się.")
            console.error(error);
        } finally {
            setLoading(false)
        }
    ...
    return (
        <View className="flex-1">
            ...
            <Modal
                visible={profilePictureSubmitted}
                animationType="fade"
                transparent={true}
                onRequestClose={() => setProfilePictureSubmitted(false)}
            >
                <TouchableWithoutFeedback onPress={() => setProfilePictureSubmitted(false)}>
